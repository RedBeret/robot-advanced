...
Features:
- Session management with automatic auth
- Request/response logging
- JSON schema validation
- Response chaining (use values from one response in the next)
...
class APIFramework:
    ...
    def get_last_response(self) -> dict:
        """Return the most recent response for further assertions."""
        return self._last_response

    @keyword("Save Response Value")
    def save_response_value(self, key: str, json_path: str):
        """Save a value from the last response for use in later requests.

        json_path uses dot notation: 'data.user.id'
        """
    ...
    def response_should_contain_key(self, key: str):
        """Assert the last response body contains a key."""
    ...
    def response_value_should_be(self, json_path: str, expected):
        """Assert a value in the response body."""
    ...
    def response_time_should_be_below(self, max_ms: int):
        """Assert the last request completed within time limit."""
    ...
    def _request(self, method: str, path: str, body: dict = None,
                 expected_status: int = 200, retries: int = 2) -> dict:
        ...
        for attempt in range(retries + 1):
            try:
                ...
                if e.code != expected_status:
                    raise AssertionError(
                        f"Expected status {expected_status}, got {e.code}")
                return self._last_response
            except URLError:
                if attempt < retries:
                    time.sleep(1)
